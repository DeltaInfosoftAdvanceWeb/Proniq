...
export default function ConstructionHologram() {
    ...
    return (
        <section ref={containerRef} className="relative z-50 h-screen w-full bg-transparent overflow-hidden flex flex-col items-center justify-center perspective-[1000px]">
            ...
            <div ref={screenRef} className="relative z-50 w-full max-w-6xl aspect-[4/5] md:aspect-video flex items-center justify-center transform-style-3d mt-10 md:mt-32">
                <div className="relative w-full h-full bg-slate-950/60 backdrop-blur-xl border border-teal-500/40 rounded-xl p-3 md:p-5 shadow-[0_0_50px_rgba(20,184,166,0.15)] overflow-hidden flex flex-col [mask-image:linear-gradient(to_bottom,black_80%,transparent_100%)]">
                    {/* Grid effect overlay */}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-12 md:grid-rows-6 gap-3 md:gap-4 relative z-10 flex-1 min-h-0 overflow-y-auto md:overflow-visible">
                        ...
                        {/* KPI Cards (Top Row) */}
                        <div className="col-span-1 md:col-span-3 md:row-span-2 bg-slate-900/50 border border-teal-500/20 rounded-lg p-3 flex flex-col justify-between group hover:border-teal-500/40 transition-colors h-24 md:h-auto">
                            <div className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Total Budget</div>
                            <div className="text-lg md:text-2xl font-bold text-white">$2.4M</div>
                            <div className="flex items-center gap-2 text-[10px]">
                                <span className="text-emerald-400 flex items-center">▲ 12%</span>
                                <span className="text-slate-500">vs last month</span>
                            </div>
                            <div className="w-full bg-slate-800 h-1 mt-auto md:mt-2 rounded-full overflow-hidden">
                                <div className="bg-teal-500 h-full w-[75%] shadow-[0_0_10px_rgba(20,184,166,0.5)]" />
                            </div>
                        </div>

                        <div className="col-span-1 md:col-span-3 md:row-span-2 bg-slate-900/50 border border-teal-500/20 rounded-lg p-3 flex flex-col justify-between group hover:border-teal-500/40 transition-colors h-24 md:h-auto">
                            <div className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Schedule Variance</div>
                            <div className="text-lg md:text-2xl font-bold text-white">-3 Days</div>
                            <div className="flex items-center gap-2 text-[10px]">
                                <span className="text-emerald-400 flex items-center">● On Track</span>
                            </div>
                            <div className="w-full bg-slate-800 h-1 mt-auto md:mt-2 rounded-full overflow-hidden">
                        ...
                        {/* Main Chart (Right Side on Tablet/Desktop, Stacked on Mobile) */}
                        <div className="col-span-2 md:col-span-6 md:row-span-4 bg-slate-900/50 border border-teal-500/20 rounded-lg p-4 relative overflow-hidden flex flex-col min-h-[200px]">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <div className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider mb-1">Cost vs. Value Earned</div>
                            ...
                            <div className="flex-1 relative flex items-end justify-between gap-2 px-2 pb-2">
                                ...
                                {/* Chart Bars/Area */}
                                {[35, 45, 40, 60, 55, 70, 65, 85, 80, 95].map((h, i) => (
                                    <div key={i} className="relative w-full h-full flex items-end group/bar">
                                        <div
                                            style={{ height: `${h}%` }}
                                            className="w-full bg-gradient-to-t from-teal-500/20 to-teal-400/60 rounded-t-sm relative transition-all duration-500 group-hover/bar:to-teal-300/80"
                                        >
                                ...
                                <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible" preserveAspectRatio="none">
                                    <path
                                        d="M0,100 C20,80 50,90 80,60 S120,70 160,40 S200,50 240,20"
                                        fill="none"
                                        stroke="#f43f5e"
                                        strokeWidth="2"
                        ...
                        <div className="col-span-2 md:col-span-6 md:row-span-4 bg-slate-900/50 border border-teal-500/20 rounded-lg p-0 overflow-hidden flex flex-col min-h-[200px]">
                            <div className="p-3 border-b border-slate-800 bg-slate-800/30 flex justify-between items-center">
                                <span className="text-slate-300 text-[10px] font-bold uppercase">Active Work Orders</span>
                                <span className="text-teal-400 text-[9px]">VIEW ALL</span>
                            </div>
                            <div className="flex-1 overflow-hidden overflow-x-auto">
                                <table className="w-full text-left border-collapse min-w-[300px]">
                                    ...
                                    <tbody className="text-[10px] text-slate-300">
                                        {[
                                            { id: "WO-204", task: "Foundation Pour", status: "In Progress", progress: 65, color: "text-blue-400" },
                                            { id: "WO-205", task: "Steel Reinforcement", status: "Pending", progress: 0, color: "text-slate-400" },
                                            { id: "WO-206", task: "Elec. Rough-in", status: "Completed", progress: 100, color: "text-emerald-400" },
                                            { id: "WO-207", task: "HVAC Ducting", status: "Delayed", progress: 45, color: "text-amber-400" },
                                        ].map((row, i) => (
                                            <tr key={i} className="border-b border-slate-800/50 hover:bg-teal-500/5 transition-colors cursor-default group/row">
                                                <td className="p-2 font-mono text-slate-500 group-hover/row:text-teal-300">{row.id}</td>
                                                ...
                                                <td className="p-2 text-right">
                                                    <div className="w-12 h-1 bg-slate-800 rounded-full inline-block align-middle ml-2">
                                                        <div style={{ width: `${row.progress}%` }} className={`h-full rounded-full ${row.progress === 100 ? 'bg-emerald-500' : 'bg-teal-500'}`} />
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                        ...
                        {/* Bottom Right Stats */}
                        <div className="col-span-2 md:col-span-6 md:row-span-2 bg-slate-900/50 border border-teal-500/20 rounded-lg p-3 flex items-center justify-around h-20 md:h-auto">
                            <div className="text-center">
                                ...
                                <div className="text-[8px] text-slate-600">Last 45 Days</div>
                            </div>
                            <div className="w-px h-8 bg-slate-800" />
                            <div className="text-center">
                                <div className="text-[9px] text-slate-500 uppercase mb-1">Workforce</div>
                                <div className="text-xl font-bold text-white">142</div>
                                <div className="text-[8px] text-slate-600">On Site Today</div>
                            </div>
                            <div className="w-px h-8 bg-slate-800" />
                            <div className="text-center">
                                <div className="text-[9px] text-slate-500 uppercase mb-1">Equipment</div>
                                <div className="text-xl font-bold text-blue-400">94%</div>
                                <div className="text-[8px] text-slate-600">Utilization</div>
                            </div>
                        </div>
                    ...
                    <div className="absolute inset-0 bg-teal-500/5 blur-3xl -z-10 rounded-full opacity-50 pointer-events-none" />
                </div>
            </div>
            ...
            <div ref={contentRef} className="absolute bottom-10 md:bottom-20 z-60 text-center px-4 max-w-2xl">
                <h2 className="text-3xl md:text-5xl font-bold text-white mb-4 drop-shadow-lg">
                    Integrated <span className="text-teal-400">AI</span> Automation
                </h2>
                <p className="text-slate-300 text-lg md:text-xl drop-shadow-md">
                    Let built‑in AI read updates, suggest next steps, and automate repetitive tasks so approvals and actions happen in just a few clicks.
                </p>
